...
class DeviationOnehotReward(AbstractReward):
    ...
    def _compute_sentence_reward(self, reference: List[str], hypothesis: List[str]) -> float:
        ...
        deviation_vec = self.text2vec_deviation(hypothesis)
        # target_index でどれだけ大きいか
        diff_value = deviation_vec[self.target_index]
        return diff_value
