...
from utils import batch_iter, read_corpus_de_en, notify_slack_if_need, log_decode_to_tensorboard, list_dict_update
...
def train_mle(args: Dict):
    ...
    device = torch.device(f"cuda:{args['--cuda-index']}" if args['--cuda'] else "cpu")
    ...
    log_decode_to_tensorboard(-1, DECODE_LOG_INDEXES, writer, args, dev_data=dev_data)
        ...
        for src_sents, tgt_sents in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            ...
            if train_iter % valid_niter == 0:
                ...
                if 'top_hyps' in eval_info:
                    # log to tensorboard
                    log_decode_to_tensorboard(train_iter, DECODE_LOG_INDEXES, writer, eval_info=eval_info)
